...
public class TextChannelBehavior {
    public static void behave(long channelId){
        String categoryName = "TextChannel";

        // Update text channel ever
        DatabaseStatisticsService.updateStatistics(categoryName, -1, String.valueOf(channelId), null, 1, response -> {
            if(response instanceof Exception){
                LogUtils.fatalError("Error while increment text channel ever. Error: '" + ((Exception)response).getMessage() + "'");
                return;
            }
            LogUtils.info("Successfully increment text channel ever");
        });

        // Update text channel month
        DatabaseStatisticsService.updateStatistics(categoryName + "Month", -1, String.valueOf(channelId), CommonUtils.statisticsManager.formatDate(TimeUnitEnum.MONTH), 1, response -> {
            if(response instanceof Exception){
                LogUtils.fatalError("Error while increment text channel month. Error: '" + ((Exception)response).getMessage() + "'");
                return;
            }
            LogUtils.info("Successfully increment text channel month");
        });
    }
}
